...
// start of indirect addresses for sequential writing of control parameters
#define CTRL_WRITE_START 		224     // read next 14 bytes, up to address 0xED
#define CTRL_WRITE_LEN			14		// length of sequence of controller parameters
...
{
public:
    XM430_bus(UART_HandleTypeDef* uart, GPIO_TypeDef* port, uint16_t pin);
    ~XM430_bus();
...
//    void bWrite();
	...
	void SetIndirectAddress(uint8_t id, uint16_t indirect_addr, uint16_t desired_addr);
