...
//extern RawSerial pc;
XM430_bus::XM430_bus(UART_HandleTypeDef* uart, GPIO_TypeDef* port, uint16_t pin)
{
    baudrate = 4000000; //4000000;
...
//    HAL_Delay(300); // TODO: how long does this need to be?
...
void XM430_bus::iWrite(uint8_t id, uint16_t address, uint8_t param[], uint8_t paramLength)
{
    ...
    iPacket[5] = (5 + paramLength);  //Packet Length LEN_L
    iPacket[6] = 0x00;          //Packet Length LEN_H
...
void XM430_bus::sWrite(uint8_t data_length, uint16_t address, uint8_t param[], uint8_t paramLength)
{
    ...
    iPacket[5] = (7 + paramLength);  //Packet Length LEN_L
    iPacket[6] = 0x00;          //Packet Length LEN_H
...
void XM430_bus::sendIPacket()
{
    ...
    //Send packet
    HAL_GPIO_WritePin(rtswitch_port, rtswitch_pin, GPIO_PIN_SET); // set switch output to "transmit"
    delay_us(20); //15us from Adi // 20us from David
    HAL_StatusTypeDef status;
    status = HAL_UART_Transmit(sbus, iPacket, iPacketLength, timeout_ms);
    if (status!=HAL_OK){
      printf("TX error condition %d\n\r", status);
    }
    delay_us(10);
    HAL_GPIO_WritePin(rtswitch_port, rtswitch_pin, GPIO_PIN_RESET); // set switch output to "receive"
...
//    printf("%x, %x, %x, %x, %x, %x, %x, %x, %x, %x, %x, %x\n\r", rPacket[0],rPacket[1],rPacket[2],rPacket[3],rPacket[4],rPacket[5],rPacket[6],rPacket[7],rPacket[8],rPacket[9],rPacket[10],rPacket[11],rPacket[12]);
    if (status != HAL_OK){
    	printf("RX error condition %d\n\r", status);
    	rPacket[8] = 0x80;
    }

}

/***** Dynamixel Protocol 2.0 Methods with DMA *****/
void XM430_bus::sendIPacket_DMA()
{
    //Send packet
    HAL_GPIO_WritePin(rtswitch_port, rtswitch_pin, GPIO_PIN_SET); // set switch output to "transmit"
    HAL_UART_Transmit_DMA(sbus, iPacket, iPacketLength);
}

void XM430_bus::getRPacket_DMA()
{
    HAL_UART_Receive_DMA(sbus, rPacket, rPacketLength);
...
void XM430_bus::SetSomething(uint8_t id, uint16_t address, uint8_t param[], uint8_t paramLength)
{
    ...
    switch(rPacket[8]) {
        case 0x00 : //No error
//            printf("No return error\n\r");
        	break;

        case 0x80 :
//            printf("Missed status packet\n\r"); // Print error
            break;

        default :   //Error occurred
            //printf("Error %d in status packet\r\n", rPacket[8]); // Print error
            break;
    }
}
...
{
    uint32_t ret_vals[idLength+1];
    if (ret_vals[idLength+1]!=0x80){
...
/***** Setting Indirect Addresses *****/
void XM430_bus::SetIndirectAddress(uint8_t id, uint16_t indirect_addr, uint16_t desired_addr)
{
	uint8_t parameter[2];
	parameter[0] = SHIFT_TO_LSB(desired_addr);
	parameter[1] = SHIFT_TO_MSB(desired_addr);
	SetSomething(id, indirect_addr, parameter, 2);
