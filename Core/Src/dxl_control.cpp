...
//	dxl_bus_2.sendIPacket_DMA();
	dxl_bus_3.sendIPacket_DMA();
//	dxl_bus_4.sendIPacket_DMA();
	while((!tx_flag_1)||(!tx_flag_3)){;}
	dxl_bus_1.getRPacket_DMA();
//	dxl_bus_2.getRPacket_DMA();
	dxl_bus_3.getRPacket_DMA();
//	dxl_bus_4.getRPacket_DMA();
	while((!rx_flag_1)||(!rx_flag_3)){;}
...
int dxl_main(void)
{
	...
	sense_can_filt.FilterID2 = 0x0F; // up to 0x0E for phalange sensors
	...
	while (1)
	{
		...
		if (SENSOR_DEBUG) {
			if(loop_count % 1000000 == 0){
				printf("Loop time: %u, %u, %u \r\n",eval_time[0], eval_time[1], eval_time[2]);
